...
internal class DataStoreManagerImpl(
    ...
    override val spotifyOAuthAccessToken: Flow<String> = settingsDataStore.data.map { it[SPOTIFY_OAUTH_ACCESS_TOKEN] ?: "" }
    override suspend fun setSpotifyOAuthAccessToken(token: String) { withContext(Dispatchers.IO) { settingsDataStore.edit { it[SPOTIFY_OAUTH_ACCESS_TOKEN] = token } } }
    override val spotifyOAuthRefreshToken: Flow<String> = settingsDataStore.data.map { it[SPOTIFY_OAUTH_REFRESH_TOKEN] ?: "" }
    override suspend fun setSpotifyOAuthRefreshToken(token: String) { withContext(Dispatchers.IO) { settingsDataStore.edit { it[SPOTIFY_OAUTH_REFRESH_TOKEN] = token } } }
    override val spotifyOAuthExpiresAt: Flow<Long> = settingsDataStore.data.map { it[SPOTIFY_OAUTH_EXPIRES_AT] ?: 0L }
    override suspend fun setSpotifyOAuthExpiresAt(expiresAt: Long) { withContext(Dispatchers.IO) { settingsDataStore.edit { it[SPOTIFY_OAUTH_EXPIRES_AT] = expiresAt } } }
    override val spotifyOAuthLoggedIn: Flow<String> = settingsDataStore.data.map { it[SPOTIFY_OAUTH_LOGGED_IN] ?: FALSE }
    override suspend fun setSpotifyOAuthLoggedIn(loggedIn: Boolean) { withContext(Dispatchers.IO) { settingsDataStore.edit { it[SPOTIFY_OAUTH_LOGGED_IN] = if (loggedIn) TRUE else FALSE } } }
