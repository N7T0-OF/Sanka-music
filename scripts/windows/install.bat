@echo off
REM SpaceKai offline installer for Windows.
...
REM   - spacekai.crt
REM   - spacekai-<version>.msix     (any .msix matching spacekai-*.msix)
REM
...
echo.
echo === SpaceKai offline installer ===
echo.

REM --- 2. Install cert to Trusted People (idempotent via certutil) -----------
if not exist "%~dp0spacekai.crt" (
    echo [ERROR] spacekai.crt not found next to install.bat
    pause
    exit /b 1
)

echo [1/3] Importing spacekai.crt to LocalMachine\TrustedPeople ...
certutil -addstore -f "TrustedPeople" "%~dp0spacekai.crt"
if %errorlevel% neq 0 (
...
set "MSIX="
for %%f in ("%~dp0spacekai-*.msix") do set "MSIX=%%f"

if not defined MSIX (
    echo [ERROR] No spacekai-*.msix found in this folder.
    pause
    exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ForceUpdateFromAnyVersion -ErrorAction Stop; exit 0 } catch { Write-Host 'In-place update failed; removing existing SpaceKai and reinstalling...'; Get-AppxPackage -Name 'Spacekai' | Remove-AppxPackage -ErrorAction SilentlyContinue; try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ErrorAction Stop; exit 0 } catch { Write-Host $_; exit 1 } }"
...
REM --- 4. Launch app ---------------------------------------------------------
echo.
echo [3/3] Launching SpaceKai ...
start "" "shell:appsFolder\Spacekai_ejp2bhxmz1qq6!Spacekai"

echo.
echo SpaceKai installed successfully.
echo To uninstall: Settings -^> Apps -^> Installed apps -^> SpaceKai -^> Uninstall
echo.
pause
endlocal
